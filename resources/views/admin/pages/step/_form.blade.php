<div class="row">
        ...
        @foreach($langs as $lang)
            <div class="tab-pane  fade @if($loop->first) show active @endif " 
                ...
                <div class="form-group ">
                    <label for="name">Step ({{ $lang->lang }}) </label>
                    <input type="text" value="{{old("step[$lang->lang ]",$item->translate($lang->lang)?->step)}}"
                        name="step[{{ $lang->lang }}]" 
                ...
                <div class="form-group mt-3 ">
                    <label for="name">Məzmun ({{ $lang->lang }}) </label>
                        <textarea class="form-control editor" name="content[{{ $lang->lang }}]"  
                            id="" cols="30" rows="10"  
                           novalidate>{{old("content",$item->translate($lang->lang)?->description)}}</textarea>
                </div>
            </div>
            @endforeach
    </div>
    ...
    <div class="col-6">
        <label for="file" class="form-label ">Şəkil <span class="text-danger">410x410</span></label>
